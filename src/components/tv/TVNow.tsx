...
export default function TVNow({ channelSlug }: { channelSlug?: string }) {
  ...
  const Item = ({ item }: { item: any }) => {
    ...
    return (
      <a href={m ? `/sport/${m.slug || m.id}` : '#'} onClick={()=>m && track('tv_program_click',{ id: m.id })} className="rounded-md border p-3 hover:bg-muted">
        <div className="flex items-center justify-between gap-2">
          <div className="font-medium">{m ? `${m.home} – ${m.away}` : item.title}</div>
          {m?.status === 'LIVE' && <span className="text-xs px-2 py-1 rounded bg-destructive text-destructive-foreground">LIVE {m?.score?.elapsed ? `• ${m.score.elapsed}'` : ''}</span>}
        </div>
