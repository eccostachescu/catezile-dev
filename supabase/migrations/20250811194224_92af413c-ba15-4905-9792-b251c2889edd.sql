...
create policy "select own profile" on public.profile for select using (auth.uid() = id or public.is_admin());
...
create policy "admin write category" on public.category for all using (public.is_admin()) with check (public.is_admin());

-- Event: public reads only PUBLISHED; admins full
create policy "public read events" on public.event for select using (status = 'PUBLISHED' or public.is_admin());
create policy "admin write events" on public.event for all using (public.is_admin()) with check (public.is_admin());

-- Countdown: public reads only APPROVED+PUBLIC; owners see theirs; admins full
create policy "public read approved countdowns" on public.countdown for select using (
  (status='APPROVED' and privacy='PUBLIC')
  or owner_id = auth.uid()
  or public.is_admin()
);
create policy "owner write countdown" on public.countdown for insert with check (owner_id = auth.uid());
create policy "owner update/delete countdown" on public.countdown for update using (owner_id = auth.uid());
create policy "admin write countdown" on public.countdown for all using (public.is_admin()) with check (public.is_admin());

-- Affiliate links + offers: admins only
create policy "admin affiliate" on public.affiliate_link for all using (public.is_admin()) with check (public.is_admin());
create policy "admin event_offer" on public.event_offer for all using (public.is_admin()) with check (public.is_admin());

-- Match/Movie: public read; admin write
create policy "public read matches" on public.match for select using (true);
create policy "admin write matches" on public.match for all using (public.is_admin()) with check (public.is_admin());
create policy "public read movies" on public.movie for select using (true);
create policy "admin write movies" on public.movie for all using (public.is_admin()) with check (public.is_admin());

-- Follow/Reminder: owner only
create policy "owner follow" on public.follow for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "owner reminder" on public.reminder for all using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Clicks: anyone can insert (server/outlinks), admins read
create policy "insert clicks" on public.click for insert with check (true);
create policy "admin read clicks" on public.click for select using (public.is_admin());
