...
-- Helper: normalize text (unaccent and lower) for building search_text
...
-- Column-specific triggers to avoid recursion
create or replace trigger _event_search_tsv after insert or update of title, description on public.event
  for each row execute function public.refresh_search_row();
create or replace trigger _match_search_tsv after insert or update of home, away, tv_channels on public.match
  for each row execute function public.refresh_search_row();
create or replace trigger _movie_search_tsv after insert or update of title, original_title, genres on public.movie
  for each row execute function public.refresh_search_row();
create or replace trigger _countdown_search_tsv after insert or update of title on public.countdown
  for each row execute function public.refresh_search_row();
...
create policy if not exists public_read_tag on public.tag for select using (true);
create policy if not exists public_read_tv_channel on public.tv_channel for select using (true);
create policy if not exists public_read_event_tag on public.event_tag for select using (true);
create policy if not exists public_read_search_index on public.search_index for select using (true);

-- admin write
create policy if not exists admin_write_tag on public.tag for all using (is_admin()) with check (is_admin());
create policy if not exists admin_write_tv_channel on public.tv_channel for all using (is_admin()) with check (is_admin());
create policy if not exists admin_write_search_index on public.search_index for all using (is_admin()) with check (is_admin());
create policy if not exists admin_write_synonym on public.search_synonym for all using (is_admin()) with check (is_admin());

-- Seed initial FTS values for existing rows
