...
create table if not exists public.url_change_log (
  ...
  entity_id uuid,
  url text not null,
  reason text not null,              -- PUBLISHED/UPDATED/UNPUBLISHED/DELETED
  created_at timestamptz default now()
);
create index if not exists idx_url_change_created on public.url_change_log(created_at);

-- Ensure sitemap_chunk exists
create table if not exists public.sitemap_chunk (
  id bigserial primary key,
  section text not null,             -- evenimente/sport/filme/countdowns/discovery/static/home
...
-- Recreate static pages view (idempotent)
...
-- Helpful indexes for sitemap sources (safe to re-run)
