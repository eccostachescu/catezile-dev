...
CREATE POLICY IF NOT EXISTS "holiday_read" ON public.holiday FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "holiday_admin_write" ON public.holiday FOR ALL USING (is_admin()) WITH CHECK (is_admin());

ALTER TABLE public.holiday_instance ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "holiday_instance_read" ON public.holiday_instance FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "holiday_instance_admin_write" ON public.holiday_instance FOR ALL USING (is_admin()) WITH CHECK (is_admin());

ALTER TABLE public.school_calendar ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "school_calendar_read" ON public.school_calendar FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "school_calendar_admin_write" ON public.school_calendar FOR ALL USING (is_admin()) WITH CHECK (is_admin());

ALTER TABLE public.exam ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "exam_read" ON public.exam FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "exam_admin_write" ON public.exam FOR ALL USING (is_admin()) WITH CHECK (is_admin());

ALTER TABLE public.exam_phase ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "exam_phase_read" ON public.exam_phase FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "exam_phase_admin_write" ON public.exam_phase FOR ALL USING (is_admin()) WITH CHECK (is_admin());
