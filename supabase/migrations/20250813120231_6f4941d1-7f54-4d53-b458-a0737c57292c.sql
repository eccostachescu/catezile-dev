-- 0031_security.sql - Comprehensive security infrastructure for CateZile.ro
...
CREATE INDEX idx_rate_limit_cleanup ON public.rate_limit(window_start) WHERE window_start < now() - interval '1 hour';
...
CREATE INDEX idx_ip_blocklist_until ON public.ip_blocklist(until) WHERE until > now();
