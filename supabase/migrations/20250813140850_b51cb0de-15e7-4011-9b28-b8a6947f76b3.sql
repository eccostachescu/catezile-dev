-- Comprehensive Security Audit and RLS Enhancement (Fixed)
...
-- Security hardening: Ensure views are properly secured (fixed loop)
DO $$
DECLARE
    view_rec RECORD;
BEGIN
    ...
    LOOP
        BEGIN
            EXECUTE format('ALTER VIEW public.%I SET (security_invoker = true)', view_rec.viewname);
            RAISE NOTICE 'Set security_invoker=true for view: %', view_rec.viewname;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not set security_invoker for view %: %', view_rec.viewname, SQLERRM;
        END;
    END LOOP;
END $$;
