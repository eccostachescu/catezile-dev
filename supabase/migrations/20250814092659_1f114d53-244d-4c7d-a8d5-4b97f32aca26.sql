...
SELECT 
  ...
  END as slug,
  now() + INTERVAL '30 days' + (EXTRACT(epoch FROM random()) * INTERVAL '60 days')::interval as starts_at,
  c.id as category_id,
  'PUBLISHED' as status,
  CASE c.slug
...
WITH sample_events AS (
  SELECT e.id, e.category_id, ROW_NUMBER() OVER (PARTITION BY e.category_id ORDER BY e.starts_at) as rn
  FROM public.event e 
